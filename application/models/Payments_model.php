...
class Payments_model extends CRM_Model
{
    ...
    public function process_payment($data, $invoiceid = '')
    {
        ...
        if (is_numeric($data['paymentmode'])) {
            ...
            if (get_option('allow_payment_amount_to_be_modified') == 0 && is_client_logged_in()) {
    ...
    public function add($data)
    {
        ...
        if ($insert_id) {
            ...
            if (is_staff_logged_in()) {
                $activity = _l('invoice_activity_payment_made_by_staff', array(
                    format_money($data['amount'], $invoice->symbol),
                    '<a href="' . admin_url('payments/payment/' . $insert_id) . '" target="_blank">#' . $insert_id . '</a>'
                ));
                $this->invoices_model->log_invoice_activity($data['invoiceid'], $activity);
            } else {
                $activity = _l('invoice_activity_payment_made_by_client', array(
                    format_money($data['amount'], $invoice->symbol),
                    '<a href="' . admin_url('payments/payment/' . $insert_id) . '" target="_blank">#' . $insert_id . '</a>'
                ));
                $this->invoices_model->log_invoice_activity($data['invoiceid'], $activity, true);
            }
    ...
    public function delete($id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            $activity = _l('invoice_activity_payment_deleted', array(
                $current->paymentid,
                format_money($current->amount, $current_invoice->symbol)
            ));
            $this->invoices_model->log_invoice_activity($invoiceid, $activity);
