                            ...
                            $text_replace = '';
                                ...
                                if (isset($_field['fromoptions'])) {
                                    ...
                                    if ($_field['key'] == '{client_country}') {
                                        $this->db->select('short_name')->from('tblclients')->join('tblcountries', 'tblcountries.country_id = tblclients.country');
                                       $country = $this->db->get()->row();
                                       if($country){
                                        $text_replace = $country->short_name;
                                       }
                                    } else {
                                    ...
                                    if ($_field['key'] == '{ticket_department}') {
                                        $this->db->select('tbldepartments.name')->from('tbltickets')->join('tbldepartments', 'tbldepartments.departmentid = tbltickets.department');
                                        $this->db->where('ticketid', $ticketid);
                                        $text_replace = $this->db->get()->row()->name;
                                    } else if ($_field['key'] == '{ticket_status}') {
                                        $this->db->select('tblticketstatus.name')->from('tbltickets')->join('tblticketstatus', 'tblticketstatus.ticketstatusid = tbltickets.status', 'left');
                                        $this->db->where('ticketid', $ticketid);
                                        $text_replace = $this->db->get()->row()->name;
                                    } else if ($_field['key'] == '{ticket_service}') {
                                        if ($use_services == '1') {
                                            $this->db->select('tblservices.name')->from('tbltickets')->join('tblservices', 'tblservices.serviceid = tbltickets.service', 'left');
                                            $this->db->where('ticketid', $ticketid);
                                    ...
                                    } else if ($_field['key'] == '{ticket_message}' && ($template->slug == 'ticket-reply-to-admin' || $template->slug == 'ticket-reply')) {
                                    ...
                                    if ($taskid !== false) {
                                        ...
                                        } else if($_field['key'] == '{task_user_take_action}'){
                                            if(is_client_logged_in()){
                                                $text_replace = get_client_full_name(get_client_user_id());
                                            } else {
                                                $text_replace = get_staff_full_name(get_staff_user_id());
                                            }
                                        } else {
                                            $this->db->where('id', $taskid);
                                    ...
                                    if ($proposalid !== false) {
                                        if ($_field['key'] == '{proposal_link}') {
                                            ...
                                            if ($_row) {
                                                $text_replace = '<a href="' . site_url('viewproposal/' . $proposalid . '/' . $_row->hash) . '" target="_blank">' . $_row->subject . '</a>';
                                            }
                                        } else {
                                            $this->db->where('id', $proposalid);
    ...
    public function get_available_merge_fields()
    {
        $available_merge_fields = array(
            ...
            array(
                'tasks' => array(
                    array(
                        'name'=>'Staff/Customer who take action on task',
