...
class Leads_model extends CRM_Model
{
    ...
    public function add($data)
    {
        ...
        if ($insert_id) {
            ...
            $this->log_lead_activity($insert_id, _l('not_lead_activity_created', get_staff_full_name()));
            if (isset($custom_fields)) {
                handle_custom_fields_post($insert_id, $custom_fields);
            }
            if ((!empty($data['assigned']) && $data['assigned'] != 0) && $data['assigned'] != get_staff_user_id()) {
                add_notification(array(
                    'description' => _l('not_assigned_lead_to_you', array(
                ...
                $activity = _l('not_lead_activity_assigned_to', array(
                    get_staff_full_name(),
                    '<a href="' . admin_url('profile/' . $data['assigned']) . '">' . get_staff_full_name($data['assigned']) . '</a>'
                ));
                $this->log_lead_activity($insert_id, $activity);
            }
            return $insert_id;
        }
        return false;
    ...
    public function update($data, $id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            if ($current_status_id != $data['status']) {
                ...
                $activity        = _l('not_lead_activity_status_updated', array(
                    get_staff_full_name(),
                    $current_status,
                    $new_status_name
                ));
                $this->log_lead_activity($id, $activity);
            }
    ...
    public function mark_as_lost($id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            $this->log_lead_activity($id, _l('not_lead_activity_marked_lost'));
    ...
    public function unmark_as_lost($id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            $this->log_lead_activity($id, _l('not_lead_activity_unmarked_lost'));
    ...
    public function mark_as_junk($id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            $this->log_lead_activity($id, _l('not_lead_activity_marked_junk'));
    ...
    public function unmark_as_junk($id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            $this->log_lead_activity($id, _l('not_lead_activity_unmarked_junk'));
    ...
    public function delete_lead_attachment($id)
    {
        ...
        if ($attachment) {
            if (unlink(LEAD_ATTACHMENTS_FOLDER . $attachment->leadid . '/' . $attachment->file_name)) {
                ...
                $this->log_lead_activity($attachment->leadid, _l('not_lead_activity_attachment_deleted'));
            }
            return true;
        }
        return false;
    ...
    public function add_note($data)
    {
        ...
        if ($insert_id) {
            if (isset($contacted_date)) {
                $this->db->where('id', $data['leadid']);
                $this->db->update('tblleads', array(
                    'lastcontact' => $contacted_date . ' '.date('H:i:s')
                ));
                if ($this->db->affected_rows() > 0) {
                    $activity = _l('not_lead_activity_contacted', array(
                        get_staff_full_name(),
                        _d($contacted_date)
                    ));
                    $this->log_lead_activity($data['leadid'], $activity);
                }
            }
            return $insert_id;
        }
