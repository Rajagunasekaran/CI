...
class Clients extends Clients_controller
{
    ...
    public function projects(){
        ...
        if (!has_customer_permission('projects')) {
            redirect(site_url());
        }

        $data['projects'] = $this->projects_model->get('',array('clientid'=>get_client_user_id()));
    ...
    public function project($id){
        ...
        $project =  $this->projects_model->get($id,array('clientid'=>get_client_user_id()));
        ...
        if($this->input->get('taskid')){
            $data['view_task'] = $this->tasks_model->get($this->input->get('taskid'),array('rel_id'=>$project->id,'rel_type'=>'project'));
        }
        ...
        $data['group'] = $group;
        $data['invoices'] = $this->invoices_model->get('',array('clientid'=>get_client_user_id(),'project_id'=>$id));
        ...
        if ($this->input->get('discussion_id')) {
            $data['discussion_user_profile_image_url'] = client_profile_image_url(get_client_user_id());
    ...
    public function remove_task_comment($id){
        ...
        echo json_encode(array('success'=>$this->tasks_model->remove_comment($id)));
    }
    ...
    public function viewproposal($id, $hash)
    {
        ...
        if ($this->input->post()) {
            ...
            switch ($action) {
                ...
                $data               = $this->input->post();
                $data['proposalid'] = $id;
                $this->proposals_model->add_comment($data);
                redirect($this->uri->uri_string());
                break;
                case 'accept_proposal':
                $success = $this->proposals_model->mark_action_status(3, $id);
                if ($success) {
                    redirect($this->uri->uri_string(), 'refresh');
                }
                break;
                case 'decline_proposal':
                $success = $this->proposals_model->mark_action_status(2, $id);
                if ($success) {
                    redirect($this->uri->uri_string(), 'refresh');
                }
                break;
    ...
    public function viewinvoice($id = '', $hash = '')
    {
        ...
        if ($this->input->post('invoicepdf')) {
            ...
            $pdf->Output(strtoupper(slug_it($invoice_number)) . '.pdf', 'D');
            die();
        }
    ...
    public function viewestimate($id, $hash)
    {
        ...
        if ($this->input->post('estimatepdf')) {
            ...
            $pdf->Output(strtoupper(slug_it($estimate_number)) . '.pdf', 'D');
            die();
        }
    ...
    public function profile()
    {
        ...
        if ($this->input->post('profile')) {
            ...
            if ($this->form_validation->run() !== FALSE) {
                handle_client_profile_image_upload();
                $data = $this->input->post();
    ...
    public function remove_profile_image()
    {
        ...
        if (file_exists(CLIENT_PROFILE_IMAGES_FOLDER . get_client_user_id())) {
            delete_dir(CLIENT_PROFILE_IMAGES_FOLDER . get_client_user_id());
        }
        ...
        if ($this->db->affected_rows() > 0) {
            redirect(site_url('clients/profile/' . get_client_user_id()));
        }
    }
    ...
    public function register()
    {
        ...
        if ($this->form_validation->run() !== FALSE) {
            if ($this->input->post()) {
                $clientid = $this->clients_model->add($this->input->post(), true);
                if ($clientid) {
    ...
    public function client_email_exists($email = '')
    {
        ...
        if ($this->input->post() && $this->input->is_ajax_request()) {
            ...
            if ($total_rows == 0) {
                $this->form_validation->set_message('client_email_exists', _l('auth_reset_pass_email_not_found'));
                return false;
            }
            return true;
        }
